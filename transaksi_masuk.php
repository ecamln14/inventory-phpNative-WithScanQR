...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $no         = generateNoTrx($conn, 'masuk');

    if ($jumlah <= 0) {
        $pesan = ['type'=>'danger','text'=>'Jumlah harus lebih dari 0!'];
    } else {
        $conn->query("INSERT INTO transaksi (no_transaksi,tipe,barang_id,jumlah,harga_satuan,total_harga,keterangan,tanggal) VALUES ('$no','masuk',$barang_id,$jumlah,$harga,$total,'$ket','$tgl')");
...
if (isset($_GET['hapus'])) {
    $id = (int)$_GET['hapus'];
    $trx = $conn->query("SELECT * FROM transaksi WHERE id=$id AND tipe='masuk'")->fetch_assoc();
    if ($trx) {
...
$barangList = $conn->query("SELECT id,kode_barang,nama_barang,stok,satuan,harga_beli FROM barang ORDER BY nama_barang");
$riwayat    = $conn->query("SELECT t.*,b.nama_barang,b.kode_barang FROM transaksi t JOIN barang b ON t.barang_id=b.id WHERE t.tipe='masuk' ORDER BY t.created_at DESC LIMIT 20");
$noTrxBaru  = generateNoTrx($conn, 'masuk');
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<div class="layout">
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="transaksi_masuk.php" class="nav-item active"><i class="fa-solid fa-arrow-down"></i> Barang Masuk</a>
...
<div style="display:grid;grid-template-columns:360px 1fr;gap:20px;align-items:start">
    <div class="card">
        <div class="card-header"><span class="card-title"><i class="fa-solid fa-arrow-down" style="color:var(--success)"></i> Input Barang Masuk</span></div>
        <div class="card-body">
            <form method="POST">
                ...
                <div style="background:var(--primary-light);border-radius:8px;padding:12px;margin-bottom:12px;display:flex;justify-content:space-between">
                    <span style="font-weight:600;color:var(--primary)">Total</span>
                    <span id="total_preview" style="font-weight:700;color:var(--primary)">Rp 0</span>
                </div>
                <div class="form-group" style="margin-bottom:16px">
                    <label>Keterangan</label>
                    <textarea name="keterangan" class="form-control"></textarea>
                </div>
                <button type="submit" class="btn btn-success" style="width:100%"><i class="fa-solid fa-floppy-disk"></i> Simpan</button>
            </form>
        </div>
    </div>

    <div class="card">
        <div class="card-header"><span class="card-title">Riwayat Barang Masuk</span></div>
...
function isiHarga(sel) {
    document.getElementById('harga_satuan').value = sel.options[sel.selectedIndex].dataset.harga || 0;
    hitungTotal();
}
</script>
